...
function initEditorForCMSMS(editor, currentMode) {
	// Setup the editor for editing templates
	if (currentMode === 'smarty') {
		// First, create a hidden textarea that CMSMS will use to save the data
		$('#template_c .pageoverflow .pageinput:first').prepend('<textarea style="display: none" class="cms_textarea m1_contents" id="content" name="m1_contents" data-cms-lang="smarty"></textarea>');
		// Now start out with filling the content of this textarea with the content of Ace Editor
		$('textarea.m1_contents').html(htmlEncode(editor.getSession().getValue()));
		// Make sure the editor updates this textarea when making changes.
	...
	if (currentMode === 'css') {
		// First, create a hidden textarea that CMSMS will use to save the data
		$(editor.container).parent().prepend('<textarea style="display: none" class="cms_textarea m1_content" id="stylesheet" name="m1_content" data-cms-lang="css"></textarea>');
		// Now start out with filling the content of this textarea with the content of Ace Editor
		$('#stylesheet').html(htmlEncode(editor.getSession().getValue()));
		// Make sure the editor updates this textarea when making changes.
		editor.on('input', function(){
			$('#stylesheet').html(htmlEncode(editor.getSession().getValue()));
		});
	...
	if (currentMode === 'php') {
		// First, create a hidden textarea that CMSMS will use to save the data
		$(editor.container).parent().prepend('<textarea name="code" id="udtcode" class="cms_textarea code" data-cms-lang="php" style="display: none"></textarea>');
		// Now start out with filling the content of this textarea with the content of Ace Editor
		$('#udtcode').html(htmlEncode(editor.getSession().getValue()));
		// Make sure the editor updates this textarea when making changes.
		editor.on('input', function(){
			$('#udtcode').html(htmlEncode(editor.getSession().getValue()));
		});
