...
function addToolBar(editor, moduleDir, AceModes, currentMode, cssPrefMode, prefFontSize, selectedTheme) {
	
	// Override 'HTML' mode with Smarty mode
	if (currentMode == 'html') {currentMode = 'smarty'};
	
	// Create the toolbar DIV
	$('<div id="AceToolbar" class="theme-'+selectedTheme+'"></div>').insertBefore($(editor.container));
	
	// Set the toolbar width to the editor width
	$('#AceToolbar').css('width', $('.ace_editor').outerWidth());

	// Create the bottombar DIV
	$(editor.container).parent().append('<div id="AceBottombar" class="theme-'+selectedTheme+'"></div>');
	...
	$('#AceToolbar').append('<div class="AceTextSize"><input type="range" min="8" max="20" value="'+prefFontSize+'" step="1" id="AceTextSize"></div>');
...
function initKeyBindings(editor) {
	...
	exec: function() {
		...
		$('#m1_apply').trigger('click');
	},
	readOnly: true
	});
}

// Special handling for FormBuilder
window.addEventListener('load', function(){
	var bodyClass = document.getElementsByTagName('body')[0].className;
	if (~bodyClass.indexOf('FormBuilder')) {
		// This is a FormBuilder page
		var pageTabs = document.getElementById('page_tabs').children;
		console.log(pageTabs);
		for (i = 0; i < pageTabs.length; i++) {
			(function(_i){
				pageTabs[_i].addEventListener('click', function(){
					// Set the toolbars width to the editor width
					// setTimeout(function(){
						$('#AceBottombar, #AceToolbar').css('width', $('.ace_editor').outerWidth());
						console.log('Toolbars sizes reset');
					// },500);
				});
			})(i);
		}
